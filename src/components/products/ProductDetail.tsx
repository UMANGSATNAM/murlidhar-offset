        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          >
            ...
            <div className="space-y-3">
              ...
              {/* Bulk Order Callout */}
              {priceConfig && priceConfig.quantity >= 100 && (
                <motion.div
                ...
                >
                  ...
                  <div>
                    <p className="text-xs font-semibold text-green-700">Bulk Discount Applied!</p>
                    <p className="text-[10px] text-green-600">
                      You&apos;re saving extra on this bulk order
                    </p>
                  </div>
                </motion.div>
              )}
              ...
              {/* Bulk Order Info */}
              {priceConfig && priceConfig.quantity < 100 && (
                <div className="flex items-center gap-2 p-3 rounded-lg bg-gold/5 border border-gold/15">
                  <Info className="h-4 w-4 text-gold shrink-0" />
                  <span className="text-xs font-medium text-foreground">
                    Bulk order? Order 100+ pcs and save up to {product.quantityPrices.length > 0 ? Math.max(...product.quantityPrices.map(t => t.discount)) : 20}%!
                  </span>
                </div>
              )}

              {/* Estimated Savings Display */}
              {priceConfig && hasDiscount && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="flex items-center gap-2 p-3 rounded-lg bg-green-50 border border-green-200"
                >
                  ...
                  <div>
                    <p className="text-xs font-semibold text-green-700">
                      You save ₹{((product.comparePrice! - product.basePrice) * priceConfig.quantity).toLocaleString('en-IN')}!
                    </p>
                    <p className="text-[10px] text-green-600">{discountPercent}% off compared to MRP</p>
                  </div>
                </motion.div>
              )}
                  ...
                  {product.variantOptions.map((option) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">{option.label}</p>
                        <p className="text-sm font-medium text-foreground truncate">{option.values.join(', ')}</p>
                      </div>
                    </div>
                  ))}
                  {[
                    { label: 'Minimum Order', value: `${product.minQty} pieces` },
                    { label: 'Maximum Order', value: `${product.maxQty.toLocaleString('en-IN')} pieces` },
                    { label: 'Production Time', value: `${product.productionDays} business days` },
                    { label: 'Customizable', value: product.isCustomizable ? 'Yes' : 'No' },
                  ].map((spec) => (
                    <div
                      key={spec.label}
                      className="flex items-center gap-3 p-3 rounded-lg bg-gold/5 border border-gold/10"
